...
images_path = "/data/AFLW/aflw/data/flickr/"
storing_path = "/data/AFLW/cropped/227/"
...
def main():
    ...
    conn = sqlite3.connect('/data/AFLW/aflw/data/aflw.sqlite')
    c = conn.cursor()
    ...
    for row in c.execute(query_string):
        ...
        if(os.path.isfile(input_path)  == True):
            print(input_path)
            imageColor = cv2.imread(input_path, 1)
            #cv2.namedWindow('image', cv2.WINDOW_NORMAL)
            #cv2.imshow('image',imageColor)
            #cv2.waitKey(0)
            #cv2.destroyAllWindows()
            #image = cv2.imread(input_path, 0) #load in grayscale
            image = cv2.cvtColor(imageColor, cv2.COLOR_BGR2GRAY) #load the colour version
            ...
            image_cropped = np.copy(imageColor[face_y:face_y+face_h, face_x:face_x+face_w])

            #cv2.namedWindow('image_cropped', cv2.WINDOW_NORMAL)
